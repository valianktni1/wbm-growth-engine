...
function automationCard(x){return `<section class="automation"><header><strong>${esc(x.kind.replaceAll("_"," "))}</strong><span class="status ${esc(x.status)}">${x.approved_at?"approved":x.approval_required?"needs approval":x.status}</span></header><small>${niceTime(x.scheduled_for)}</small><p>${esc(x.body)}</p><footer>${x.approval_required&&!x.approved_at&&x.status==="scheduled"?`<button class="secondary" data-approve="${x.id}">Approve</button>`:""}</footer></section>`}
