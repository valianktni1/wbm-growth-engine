...
async function renderGaps() {
  ...
  workspace.innerHTML=`<div class="page-heading"><div><small class="eyebrow">MAKE ROOM FOR MORE BOOKINGS</small><h1>Fill my empty dates</h1><p>Choose dates you want to promote, review matching enquiries, and prepare a post.</p></div></div>
    ...
    <section class="panel gap-controls"><label>Look ahead<select id="gap-horizon"><option value="3">Three months</option><option value="6">Six months</option></select></label><label>Show dates<select id="gap-weekdays"><option value="weekend">Saturdays & Sundays</option><option value="all">Every day</option><option value="weekday">Monday to Friday</option></select></label><button class="primary" id="gap-refresh">Check Booking live</button></section>
    ...
    <div id="gap-calendar" aria-live="polite">Checking availability…</div><div id="gap-compose"></div>`;
  ...
  document.querySelector('#gap-horizon').onchange=()=>loadGapCalendar().catch(e=>toast(e.message));
...
async function loadGapCalendar() {
  const id=++gapRequest;
  const months=Number(document.querySelector('#gap-horizon').value);
  const start=londonDate();
  ...
  // Inclusive range is capped at 184 days by the integration.
  const cap=new Date(new Date(start+'T12:00:00Z').getTime()+183*86400000);
  const end=new Date(Math.min(endDate,cap)).toISOString().slice(0,10);
  gapCalendar=null;gapSelected.clear();
...
function drawGapCalendar() {
  ...
  document.querySelector('#gap-calendar').innerHTML=`<div class="gap-summary"><strong>${free.length} available dates</strong><span>${days.filter(d=>d.status==='booked').length} booked · ${days.filter(d=>d.status==='blocked').length} blocked in this filter</span><small>Checked ${niceTime(gapCalendar.checked_at)} · checked again when you prepare or copy a campaign</small></div>
...
function showGapComposer(){
  ...
  form.onsubmit=async event=>{event.preventDefault();const button=form.querySelector('button');button.disabled=true;try{
    ...
    const payload={dates:selected,channel:fields.channel,package_id:fields.package_id||null};
