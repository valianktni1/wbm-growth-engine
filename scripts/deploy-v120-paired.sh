...
docker tag "$(docker inspect -f '{{.Image}}' wbm-growth-engine-app)" "wbm-growth-engine:before-v120-$RELEASE_STAMP"
docker tag "$(docker inspect -f '{{.Image}}' bookingsystem2026-app)" "bookingsystem2026:before-v839-$RELEASE_STAMP"
...
docker compose run --rm --no-deps --entrypoint python app -c "from pathlib import Path; assert '2026.09.07-today-and-performance-v1.2.0' in Path('/app/app/config.py').read_text(), 'Growth image is not V1.2.0'"
docker compose -f "$BOOKING_STACK/compose.yaml" run --rm --no-deps --entrypoint python app -c "from pathlib import Path; assert '2026.09.07-growth-intelligence-v8.39' in Path('/app/app/backup.py').read_text(), 'Booking image is not V8.39'"
...
backup_database wbm-growth-engine-db growthengine "$RELEASE_BACKUPS/growth-before-v120-$RELEASE_STAMP.dump"
backup_database bookingsystem2026-db bookingapp "$RELEASE_BACKUPS/booking-before-v839-$RELEASE_STAMP.dump"
wait_health() {
...
echo 'Updating Growth first so the new activity receiver is ready...'
docker compose up -d --no-deps app
wait_health 30110 2026.09.07-today-and-performance-v1.2.0
docker compose -f "$BOOKING_STACK/compose.yaml" up -d --no-deps app
wait_health 30049 2026.09.07-growth-intelligence-v8.39
...
PY
echo 'Both apps updated. Refresh Growth: Today and What works are ready.'
