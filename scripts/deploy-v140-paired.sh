...
docker tag "$(docker inspect -f '{{.Image}}' wbm-growth-engine-app)" "wbm-growth-engine:before-v140-$RELEASE_STAMP"
docker tag "$(docker inspect -f '{{.Image}}' bookingsystem2026-app)" "bookingsystem2026:before-v842-$RELEASE_STAMP"
...
docker compose run --rm --no-deps --entrypoint python app -c "from pathlib import Path; assert '2026.09.08-business-planning-v1.4.0' in Path('/app/app/config.py').read_text(), 'Growth image is not V1.4.0'"
docker compose -f "$BOOKING_STACK/compose.yaml" run --rm --no-deps --entrypoint python app -c "from pathlib import Path; assert '2026.09.08-growth-planning-v8.42' in Path('/app/app/backup.py').read_text(), 'Booking image is not V8.42'"
...
backup_database wbm-growth-engine-db growthengine "$RELEASE_BACKUPS/growth-before-v140-$RELEASE_STAMP.dump"
backup_database bookingsystem2026-db bookingapp "$RELEASE_BACKUPS/booking-before-v842-$RELEASE_STAMP.dump"
wait_health() {
...
wait_health 30110 2026.09.08-business-planning-v1.4.0
docker compose -f "$BOOKING_STACK/compose.yaml" up -d --no-deps app
wait_health 30049 2026.09.08-growth-planning-v8.42
