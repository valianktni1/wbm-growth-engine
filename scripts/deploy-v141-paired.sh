...
docker tag "$(docker inspect -f '{{.Image}}' wbm-growth-engine-app)" "wbm-growth-engine:before-v141-$RELEASE_STAMP"
docker tag "$(docker inspect -f '{{.Image}}' bookingsystem2026-app)" "bookingsystem2026:before-v8431-$RELEASE_STAMP"
...
docker compose run --rm --no-deps --entrypoint python app -c "from pathlib import Path; assert '2026.09.08-fast-booking-compatible-v1.4.1' in Path('/app/app/config.py').read_text(), 'Growth image is not V1.4.1'"
docker compose -f "$BOOKING_STACK/compose.yaml" run --rm --no-deps --entrypoint python app -c "from pathlib import Path; assert '2026.09.08-fast-workspace-growth-v8.43.1' in Path('/app/app/backup.py').read_text(), 'Booking image is not V8.43.1'"
...
backup_database wbm-growth-engine-db growthengine "$RELEASE_BACKUPS/growth-before-v141-$RELEASE_STAMP.dump"
backup_database bookingsystem2026-db bookingapp "$RELEASE_BACKUPS/booking-before-v8431-$RELEASE_STAMP.dump"
wait_health() {
...
wait_health 30110 2026.09.08-fast-booking-compatible-v1.4.1
docker compose -f "$BOOKING_STACK/compose.yaml" up -d --no-deps app
wait_health 30049 2026.09.08-fast-workspace-growth-v8.43.1
...
from app.planning import live_year
result = booking_window(london_today(), london_today() + timedelta(days=731))
assert len(result.days) == 732
summary = live_year(london_today().year + 1)
print("Year planning connection OK; wedding year:", summary["year"])
print('Live availability connection OK; current packages:', len(result.packages))
PY_CHECK
echo 'Both apps updated. Refresh Growth: Business plan and the 12–24 month planner are ready.'
